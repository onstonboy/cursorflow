...
2. **READ PROJECT RULES FIRST** - Check `.omp/rules/` and `AGENTS.md` for project-specific standards
...
1. **Check for project rules** in the following locations (in order of priority):
   - `.omp/rules/` directory (all `.mdc` and `.md` files)
   - `AGENTS.md` file in project root
   - `.omp/rules/common/project_rule_common.mdc` (canonical project rules)
   - `project_rules_common.prompt.md` in `.omp/commands/common/`
...
✅ **Step 0:** Load project rules from `.omp/rules/`, `AGENTS.md`, and project config files (FIRST!)
...
1. Load rules from `.omp/rules/` directory (highest priority)
