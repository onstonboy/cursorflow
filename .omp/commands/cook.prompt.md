...
**IMPORTANT:** After Step 0 verification, tech-specific prompt files will be available in `./.omp/commands/specify/`. These files take precedence over common files.

Before executing this workflow, you MUST have access to and reference these prompt files:

1. **Research & Analysis Prompt**
   - **Primary:** `./.omp/commands/specify/research_plan_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.omp/commands/common/research_plan_common.prompt.md`
...
2. **UI/UX Design Generator Prompt**
   - **Primary:** `./.omp/commands/specify/ui_ux_design_generator_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.omp/commands/common/ui_ux_design_generator.prompt.md`
...
3. **UI/UX Bridge Prompt**
   - **Primary:** `./.omp/commands/specify/ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.omp/commands/common/ui_ux_bridge.prompt.md`
...
4. **Implementation Plan Prompt**
   - **Primary:** `./.omp/commands/specify/implementation_plan_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.omp/commands/common/implementation_plan_common.prompt.md`
...
- Step 0.x updates under `./.omp/` (rules and command prompts only)
...
**Check and record:**
1. List all files in `./.omp/commands/specify/` (if directory exists)
2. List all files in `./.omp/rules/` (excluding subfolders `common/` and `templates/`)
3. List files in `./.omp/rules/templates/` (for stack match when generating `project-rule_*`)
...
3. **Use common rules, stack templates, and commands as base**
   - Read `./.omp/rules/common/project_rule_common.mdc` as the **contract** for project rules (full topic parity required for tech-specific output — see §0.0.2a)
   - Read `./.omp/rules/templates/README.md` and the **best-matching** `./.omp/rules/templates/*-project-rule.template.mdc` for the detected stack (e.g. `flutter-project-rule.template.mdc`, `nextjs-typescript-project-rule.template.mdc`). **Pick one tier** (Small / Medium / Big) from product context (see README); use that tier’s content as **scaffold** — expand with all topics from `project_rule_common.mdc` that the template does not already cover
   - Read the relevant common command files from `./.omp/commands/common/` (e.g. `research_plan_common.prompt.md`, `ui_ux_bridge.prompt.md`, `implementation_plan_common.prompt.md`) as the base for commands

4. **Write or update rules and commands to fit the project**
   - **Rules:** Generate or update `./.omp/rules/project-rule_[TECH]_[LANGUAGE].mdc` so that it matches the project’s tech stack, language, and conventions, using **`project_rule_common.mdc` + the matching `templates/` file (tier chosen) +** researched official/relevant docs. If **no** template exists for the stack, still meet §0.0.2a from common + research only
   - **Commands:** Generate or update tech-specific command files in `./.omp/commands/specify/` (e.g. `research_plan_[TECH]_[LANGUAGE].prompt.md`, `ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md`, etc.) so that they match the project, using the common commands and researched documentation
...
**Check process:**
1. List all files in `./.omp/commands/specify/`
...
```
./.omp/commands/specify/[FILENAME].prompt.md
```
...
**Source files to reference:**
```
./.omp/rules/common/project_rule_common.mdc
./.omp/rules/templates/README.md
./.omp/rules/templates/<matching>-project-rule.template.mdc   # when stack matches a template
```

**Generation process:**
1. Read the **entire** common project rule from `./.omp/rules/common/project_rule_common.mdc` (do not sample only the beginning).
2. Read `./.omp/rules/templates/README.md`; open the **matching** `*-project-rule.template.mdc` and extract the **chosen tier** (Small / Medium / Big) as scaffold content.
...
```
./.omp/rules/project-rule_[TECH]_[LANGUAGE].mdc
```
...
- ✅ All 4 required tech-specific command files exist in `./.omp/commands/specify/`
- ✅ Project rule file exists in `./.omp/rules/` and meets **§0.0.2a** (parity with `project_rule_common.mdc`, substantive depth, `## References consulted` when newly written); when a matching file exists under `./.omp/rules/templates/`, generation **must have used** that template’s **tier** as scaffold per §0.0.2a / §0.7
...
**Deliverable:**
- ✅ Complete set of tech-specific command files in `./.omp/commands/specify/`
- ✅ Tech-specific project rule file in `./.omp/rules/`
...
**Reference File:**
- **Primary:** `./.omp/commands/specify/ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.omp/commands/common/ui_ux_bridge.prompt.md`
...
- **Primary:** `./.omp/commands/specify/research_plan_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.omp/commands/common/research_plan_common.prompt.md`
...
**⚠️ CRITICAL: The UI/UX Design Generator Prompt includes checking `.omp/commands/common/ui_styles_reference.md`, `.omp/uiux_reference/data/`, and `.omp/uiux_reference/landing_page_prompts/` (when applicable) first to match project requirements with suitable UI styles. This step is mandatory and must be completed before proceeding.**

**File Path (from Step 0):**
- **Primary:** `./.omp/commands/specify/ui_ux_design_generator_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.omp/commands/common/ui_ux_design_generator.prompt.md`

**UI Styles Reference:**
- **Location:** `./.omp/commands/common/ui_styles_reference.md`
...
- **Location:** `./.omp/uiux_reference/data/`
...
- **Location:** `./.omp/uiux_reference/landing_page_prompts/`
...
**File Path (from Step 0):**
- **Primary:** `./.omp/commands/specify/implementation_plan_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.omp/commands/common/implementation_plan_common.prompt.md`
...
**Command Files Location:** `./.omp/commands/specify/`
**Project Rule Location:** `./.omp/rules/`
...
**Prerequisite:** Steps 0–5 are complete on disk (mandatory paperwork pack + wireframe approval + any UI/UX docs if applicable). **Steps 1–5 must not have modified the application source tree for this feature** — only `./docs/` and `.omp/` (rules/commands) as allowed in **Paperwork First**.
...
- Step 0.0: Scan `.omp/`; if rules/commands missing or don't fit project → parse user prompt, research official/relevant docs, use **`project_rule_common.mdc` + `./.omp/rules/templates/`** (README + matching `*.template.mdc` tier) → write fit rules/commands; tech-specific project rules MUST satisfy **§0.0.2a** (full parity with `project_rule_common.mdc` + web research + `## References consulted`)
- Step 0: Verify and generate tech-specific files in:
  - `./.omp/commands/specify/` (command files)
  - `./.omp/rules/` (project rule)
- Step 2: `./.omp/commands/specify/research_plan_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Step 3: `./.omp/commands/specify/ui_ux_design_generator_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Step 4: `./.omp/commands/specify/ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Step 5: `./.omp/commands/specify/implementation_plan_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Project Rule: `./.omp/rules/project-rule_[TECH]_[LANGUAGE].mdc` (or common fallback: `./.omp/rules/common/project_rule_common.mdc`); scaffolds: `./.omp/rules/templates/`
...
✅ **Step 0.0:** Scan `.omp/` for existing rules/commands; if missing or not fit → parse user prompt, research (internet, official docs), use common → write fit rules/commands
✅ **Step 0:** Verify tech-specific commands and project rule exist:
   - Commands in `./.omp/commands/specify/` → Generate missing files from `./.omp/commands/common/`
   - Project rule in `./.omp/rules/` → Generate missing file from `./.omp/rules/common/project_rule_common.mdc` **and** `./.omp/rules/templates/` (README + matching tier from `*.template.mdc`) per §0.7
...
2. ✅ **Step 0:** Verify commands and project rule for Flutter/Dart
   - Check `./.omp/commands/specify/` → Generate missing files from `./.omp/commands/common/`
   ...
   - Check `./.omp/rules/` → Generate missing file using `./.omp/rules/common/` + `./.omp/rules/templates/` per §0.7
