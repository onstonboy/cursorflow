...
2. **READ PROJECT RULES FIRST** - Check `.claude/rules/` and `CLAUDE.md` for project-specific standards
...
1. **Check for project rules** in the following locations (in order of priority):
   - `.claude/rules/` directory (all `.mdc` and `.md` files)
   - `CLAUDE.md` file in project root
   - `.claude/rules/common/project_rule_common.mdc` (canonical project rules)
   - `project_rules_common.prompt.md` in `.claude/commands/common/`
...
✅ **Step 0:** Load project rules from `.claude/rules/`, `CLAUDE.md`, and project config files (FIRST!)
...
1. Load rules from `.claude/rules/` directory (highest priority)
2. Load rules from `CLAUDE.md` file
