...
2. **READ PROJECT RULES FIRST** - Check `.kimi-code/rules/` and `AGENTS.md` for project-specific standards
...
1. **Check for project rules** in the following locations (in order of priority):
   - `.kimi-code/rules/` directory (all `.mdc` and `.md` files)
   - `AGENTS.md` file in project root
   - `.kimi-code/rules/common/project_rule_common.mdc` (canonical project rules)
   - `project_rules_common.prompt.md` in `.kimi-code/commands/common/`
...
✅ **Step 0:** Load project rules from `.kimi-code/rules/`, `AGENTS.md`, and project config files (FIRST!)
...
1. Load rules from `.kimi-code/rules/` directory (highest priority)
