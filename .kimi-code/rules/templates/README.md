...
Copy a template to `.kimi-code/rules/project-rule_<stack>.mdc` (or your team’s naming), set `alwaysApply: true` when active, and replace placeholders (`{{APP_NAME}}`, package names, routers).
...
Align all templates with `.kimi-code/rules/common/project_rule_common.mdc` for topic coverage; these files add **stack-specific** and **scale-specific** detail.
