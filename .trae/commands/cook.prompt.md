...
**IMPORTANT:** After Step 0 verification, tech-specific prompt files will be available in `./.trae/commands/specify/`. These files take precedence over common files.

Before executing this workflow, you MUST have access to and reference these prompt files:

1. **Research & Analysis Prompt**
   - **Primary:** `./.trae/commands/specify/research_plan_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.trae/commands/common/research_plan_common.prompt.md`
...
2. **UI/UX Design Generator Prompt**
   - **Primary:** `./.trae/commands/specify/ui_ux_design_generator_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.trae/commands/common/ui_ux_design_generator.prompt.md`
...
3. **UI/UX Bridge Prompt**
   - **Primary:** `./.trae/commands/specify/ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.trae/commands/common/ui_ux_bridge.prompt.md`
...
4. **Implementation Plan Prompt**
   - **Primary:** `./.trae/commands/specify/implementation_plan_[TECH]_[LANGUAGE].prompt.md` (generated in Step 0)
   - **Fallback:** `./.trae/commands/common/implementation_plan_common.prompt.md`
...
- Step 0.x updates under `./.trae/` (rules and command prompts only)
...
**Check and record:**
1. List all files in `./.trae/commands/specify/` (if directory exists)
2. List all files in `./.trae/rules/` (excluding subfolders `common/` and `templates/`)
3. List files in `./.trae/rules/templates/` (for stack match when generating `project-rule_*`)
...
3. **Use common rules, stack templates, and commands as base**
   - Read `./.trae/rules/common/project_rule_common.mdc` as the **contract** for project rules (full topic parity required for tech-specific output — see §0.0.2a)
   - Read `./.trae/rules/templates/README.md` and the **best-matching** `./.trae/rules/templates/*-project-rule.template.mdc` for the detected stack (e.g. `flutter-project-rule.template.mdc`, `nextjs-typescript-project-rule.template.mdc`). **Pick one tier** (Small / Medium / Big) from product context (see README); use that tier’s content as **scaffold** — expand with all topics from `project_rule_common.mdc` that the template does not already cover
   - Read the relevant common command files from `./.trae/commands/common/` (e.g. `research_plan_common.prompt.md`, `ui_ux_bridge.prompt.md`, `implementation_plan_common.prompt.md`) as the base for commands

4. **Write or update rules and commands to fit the project**
   - **Rules:** Generate or update `./.trae/rules/project-rule_[TECH]_[LANGUAGE].mdc` so that it matches the project’s tech stack, language, and conventions, using **`project_rule_common.mdc` + the matching `templates/` file (tier chosen) +** researched official/relevant docs. If **no** template exists for the stack, still meet §0.0.2a from common + research only
   - **Commands:** Generate or update tech-specific command files in `./.trae/commands/specify/` (e.g. `research_plan_[TECH]_[LANGUAGE].prompt.md`, `ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md`, etc.) so that they match the project, using the common commands and researched documentation
...
**Check process:**
1. List all files in `./.trae/commands/specify/`
...
```
./.trae/commands/specify/[FILENAME].prompt.md
```
...
**Check process:**
1. List all files in `./.trae/rules/`
...
**Source files to reference:**
```
./.trae/rules/common/project_rule_common.mdc
./.trae/rules/templates/README.md
./.trae/rules/templates/<matching>-project-rule.template.mdc   # when stack matches a template
```

**Generation process:**
1. Read the **entire** common project rule from `./.trae/rules/common/project_rule_common.mdc` (do not sample only the beginning).
2. Read `./.trae/rules/templates/README.md`; open the **matching** `*-project-rule.template.mdc` and extract the **chosen tier** (Small / Medium / Big) as scaffold content.
...
```
./.trae/rules/project-rule_[TECH]_[LANGUAGE].mdc
```
...
- ✅ All 4 required tech-specific command files exist in `./.trae/commands/specify/`
- ✅ Project rule file exists in `./.trae/rules/` and meets **§0.0.2a** (parity with `project_rule_common.mdc`, substantive depth, `## References consulted` when newly written); when a matching file exists under `./.trae/rules/templates/`, generation **must have used** that template’s **tier** as scaffold per §0.0.2a / §0.7
...
**Deliverable:**
- ✅ Complete set of tech-specific command files in `./.trae/commands/specify/`
- ✅ Tech-specific project rule file in `./.trae/rules/`
...
**Reference File:**
- **Primary:** `./.trae/commands/specify/ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.trae/commands/common/ui_ux_bridge.prompt.md`
...
- **Primary:** `./.trae/commands/specify/research_plan_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.trae/commands/common/research_plan_common.prompt.md`
...
**⚠️ CRITICAL: The UI/UX Design Generator Prompt includes checking `.trae/commands/common/ui_styles_reference.md`, `.trae/uiux_reference/data/`, and `.trae/uiux_reference/landing_page_prompts/` (when applicable) first to match project requirements with suitable UI styles. This step is mandatory and must be completed before proceeding.**

**File Path (from Step 0):**
- **Primary:** `./.trae/commands/specify/ui_ux_design_generator_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.trae/commands/common/ui_ux_design_generator.prompt.md`

**UI Styles Reference:**
- **Location:** `./.trae/commands/common/ui_styles_reference.md`
...
- **Location:** `./.trae/uiux_reference/data/`
...
- **Location:** `./.trae/uiux_reference/landing_page_prompts/`
...
**⚠️ CRITICAL: The UI/UX Bridge Prompt includes checking `.trae/commands/common/ui_styles_reference.md`, `.trae/uiux_reference/data/`, and `.trae/uiux_reference/landing_page_prompts/` (when applicable) first in the PRE-PHASE to match project requirements with suitable UI styles. This step is mandatory and must be completed before proceeding to Phase 1.**
...
**File Path (from Step 0):**
- **Primary:** `./.trae/commands/specify/implementation_plan_[TECH]_[LANGUAGE].prompt.md`
- **Fallback:** `./.trae/commands/common/implementation_plan_common.prompt.md`
...
**Command Files Location:** `./.trae/commands/specify/`
**Project Rule Location:** `./.trae/rules/`
...
**Prerequisite:** Steps 0–5 are complete on disk (mandatory paperwork pack + wireframe approval + any UI/UX docs if applicable). **Steps 1–5 must not have modified the application source tree for this feature** — only `./docs/` and `.trae/` (rules/commands) as allowed in **Paperwork First**.
...
- Step 0.0: Scan `.trae/`; if rules/commands missing or don't fit project → parse user prompt, research official/relevant docs, use **`project_rule_common.mdc` + `./.trae/rules/templates/`** (README + matching `*.template.mdc` tier) → write fit rules/commands; tech-specific project rules MUST satisfy **§0.0.2a** (full parity with `project_rule_common.mdc` + web research + `## References consulted`)
- Step 0: Verify and generate tech-specific files in:
  - `./.trae/commands/specify/` (command files)
  - `./.trae/rules/` (project rule)
- Step 2: `./.trae/commands/specify/research_plan_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Step 3: `./.trae/commands/specify/ui_ux_design_generator_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Step 4: `./.trae/commands/specify/ui_ux_bridge_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Step 5: `./.trae/commands/specify/implementation_plan_[TECH]_[LANGUAGE].prompt.md` (or common fallback)
- Project Rule: `./.trae/rules/project-rule_[TECH]_[LANGUAGE].mdc` (or common fallback: `./.trae/rules/common/project_rule_common.mdc`); scaffolds: `./.trae/rules/templates/`
...
✅ **Step 0.0:** Scan `.trae/` for existing rules/commands; if missing or not fit → parse user prompt, research (internet, official docs), use common → write fit rules/commands
✅ **Step 0:** Verify tech-specific commands and project rule exist:
   - Commands in `./.trae/commands/specify/` → Generate missing files from `./.trae/commands/common/`
   - Project rule in `./.trae/rules/` → Generate missing file from `./.trae/rules/common/project_rule_common.mdc` **and** `./.trae/rules/templates/` (README + matching tier from `*.template.mdc`) per §0.7
...
2. ✅ **Step 0:** Verify commands and project rule for Flutter/Dart
   - Check `./.trae/commands/specify/` → Generate missing files from `./.trae/commands/common/`
   ...
   - Check `./.trae/rules/` → Generate missing file using `./.trae/rules/common/` + `./.trae/rules/templates/` per §0.7
