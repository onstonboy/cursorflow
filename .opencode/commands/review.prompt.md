...
2. **READ PROJECT RULES FIRST** - Check `.opencode/rules/` and `AGENTS.md` for project-specific standards
...
1. **Check for project rules** in the following locations (in order of priority):
   - `.opencode/rules/` directory (all `.mdc` and `.md` files)
   - `AGENTS.md` file in project root
   - `.opencode/rules/common/project_rule_common.mdc` (canonical project rules)
   - `project_rules_common.prompt.md` in `.opencode/commands/common/`
...
✅ **Step 0:** Load project rules from `.opencode/rules/`, `AGENTS.md`, and project config files (FIRST!)
...
1. Load rules from `.opencode/rules/` directory (highest priority)
