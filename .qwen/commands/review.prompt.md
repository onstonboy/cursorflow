...
2. **READ PROJECT RULES FIRST** - Check `.qwen/rules/` and `QWEN.md` for project-specific standards
...
1. **Check for project rules** in the following locations (in order of priority):
   - `.qwen/rules/` directory (all `.mdc` and `.md` files)
   - `QWEN.md` file in project root
   - `.qwen/rules/common/project_rule_common.mdc` (canonical project rules)
   - `project_rules_common.prompt.md` in `.qwen/commands/common/`
...
✅ **Step 0:** Load project rules from `.qwen/rules/`, `QWEN.md`, and project config files (FIRST!)
...
1. Load rules from `.qwen/rules/` directory (highest priority)
2. Load rules from `QWEN.md` file
